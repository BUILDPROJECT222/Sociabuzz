...
function toNumberIDR(x){ if(x==null)return 0; if(typeof x==="number")return x;
  const n=Number(String(x).replace(/rp/gi,"").replace(/[.,](?=\d{3}\b)/g,"").replace(/[^\d.-]/g,"").trim());
...
export default async function handler(req,res){
  ...
  let body=req.body; const ctype=(req.headers["content-type"]||"").toLowerCase();
